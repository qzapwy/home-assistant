...
def setup_platform(hass, config, add_entities, discovery_info=None):
    ...
    flux = FluxSwitch(name, hass, lights, start_time, sunset_time, stop_time,
...
class FluxSwitch(SwitchDevice):
    ...
    def __init__(self, name, hass, lights, start_time, sunset_time, stop_time,
                 start_colortemp, sunset_colortemp, stop_colortemp,
                 brightness, disable_brightness_adjust, mode, interval,
                 transition):
    ...
    def find_sunset_time(self, now):
        ...
        if self._sunset_time:
            sunset = now.replace(
                hour=self._sunset_time.hour, minute=self._sunset_time.minute,
                second=0)
        else:
